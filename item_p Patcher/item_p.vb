Public Class item_p
    ...
    Default Public Property ItemIndex(Index As Integer) As Item
        Get
            Return Items(Index)
        End Get
        Set(value As Item)
    ...
    Public Class Item
        ...
        Public Overrides Function Equals(obj As Object) As Boolean
            Dim equal As Boolean = TypeOf obj Is Item
            If equal Then
                With DirectCast(obj, Item)
                    equal = (Me.RawData.Length = .RawData.Length)
                    If equal Then
                        For count = 0 To .RawData.Length
                            If Not Me.RawData(count) = .RawData(count) Then
                                equal = False
                                Exit For
                            End If
                        Next
            ...
            Return equal
        End Function
        Public Shared Operator =(ByVal Input As Item, ByVal Out As Item) As Boolean
            Return Input.Equals(Out)
        End Operator
        Public Shared Operator <>(ByVal Input As Item, ByVal Out As Item) As Boolean
            Return Not Input.Equals(Out)
        End Operator
    End Class
