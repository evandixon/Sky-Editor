...
#End Region
    ...
    ''' <returns></returns>
    Public Function GetProjectByPath(Path As String) As ProjectOld
    ...
    Public Overridable Function GetSupportedProjectTypes(Path As String) As IEnumerable(Of Type)
        If CanCreateDirectory(Path) Then
            Return PluginManager.GetInstance.GetRegisteredTypes(GetType(ProjectOld))
        Else
            Return {}
        End If
    End Function
    ...
    Public Overridable Sub CreateProject(ParentPath As String, ProjectName As String, ProjectType As Type)
        ...
        If item IsNot Nothing Then
            ...
            If q Is Nothing Then
                Dim p = ProjectOld.CreateProject(IO.Path.GetDirectoryName(Me.Filename), ProjectName, ProjectType)
    ...
    Public Overridable Sub AddExistingProject(ParentPath As String, ProjectFilename As String)
        Dim item = GetSolutionItemByPath(ParentPath)
        If item IsNot Nothing Then
            Dim p As ProjectOld = ProjectOld.OpenProjectFile(ProjectFilename)
    ...
    Public Function GetAllProjects() As IEnumerable(Of ProjectOld)
        Return GetAllProjects(SolutionNode)
    End Function

    Public Overridable Function GetAllProjects(Root As SolutionItem) As IEnumerable(Of ProjectOld)
    ...
    ''' <returns></returns>
    Public Overridable Function GetProjectsByName(Name As String) As IEnumerable(Of ProjectOld)
        Return From p In GetAllProjects() Where p.Name.ToLower = Name.ToLower
    End Function

    Public Overridable Function GetProjectsToBuild() As IEnumerable(Of ProjectOld)
    ...
    Public Overridable Async Function Build() As Task
        ...
        Dim toBuild As New Dictionary(Of ProjectOld, Boolean)
    ...
    Private Async Function BuildProjects(ToBuild As Dictionary(Of ProjectOld, Boolean), CurrentProject As ProjectOld) As Task
    ...
    Private Sub UpdateBuildLoadingStatus(toBuild As Dictionary(Of ProjectOld, Boolean))
...
#Region "Create New"
    ...
    ''' <returns></returns>
    Public Shared Function CreateSolution(SolutionDirectory As String, SolutionName As String) As SolutionOld
        Return CreateSolution(SolutionDirectory, SolutionName, GetType(SolutionOld))
    End Function
    ...
    ''' <returns></returns>
    Public Shared Function CreateSolution(SolutionDirectory As String, SolutionName As String, SolutionType As Type) As SolutionOld
        ...
        Dim output As SolutionOld = SolutionType.GetConstructor({}).Invoke({})
...
#Region "Open"
    ...
    ''' <returns></returns>
    Public Shared Function OpenSolutionFile(Filename As String) As SolutionOld
        ...
        End If
        Dim out As SolutionOld = type.GetConstructor({}).Invoke({})
        ...
        For Each item In File.Settings
            If Not Settings.ContainsKey(item.Key) Then
                ...
                If type Is Nothing Then
                    ...
                    type = GetType(Object)
                End If
                Settings.Add(item.Key, Json.Deserialize(type, item.Value.ValueJson))
            End If
        Next
        ...
        For Each item In File.Projects
            ...
            If proj Is Nothing Then
                ...
                If item.Value IsNot Nothing Then
                    ...
                    newNode.Project = ProjectOld.OpenProjectFile(IO.Path.Combine(IO.Path.GetDirectoryName(Filename), item.Value.Replace("/", "\").TrimStart("\")))
    ...
    Public Sub Save() Implements iSavable.Save
        ...
        For Each item In Me.Settings
            If item.Value IsNot Nothing Then
                ...
                value.ValueJson = Json.Serialize(item.Value)
