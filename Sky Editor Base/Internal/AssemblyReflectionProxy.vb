...
Namespace Internal
    Friend Class AssemblyReflectionProxy
        ...
        Private Property DirectoryInfo As IO.DirectoryInfo

        Public Sub LoadAssembly(AssemblyPath As String)
            Me.AssemblyPath = AssemblyPath
            Me.DirectoryInfo = New IO.FileInfo(AssemblyPath).Directory
        ...
        Private Function OnReflectionOnlyResolve(sender As Object, e As ResolveEventArgs) As Assembly
            ...
            Else
                ...
                Dim dependentAssemblyFilenameDll = IO.Path.Combine(DirectoryInfo.FullName, name.Name & ".dll")
                Dim dependentAssemblyFilenameExe = IO.Path.Combine(DirectoryInfo.FullName, name.Name & ".exe")

                If IO.File.Exists(dependentAssemblyFilenameDll) Then
                    Return Assembly.ReflectionOnlyLoadFrom(dependentAssemblyFilenameDll)
                ElseIf IO.File.Exists(dependentAssemblyFilenameExe) Then
