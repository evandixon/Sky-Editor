...
#End Region
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function FromBits(Bit1 As Boolean, Bit2 As Boolean, Bit3 As Boolean, Bit4 As Boolean, Bit5 As Boolean, Bit6 As Boolean, Bit7 As Boolean, Bit8 As Boolean) As Bit8
        Dim out As New Bit8(0)
        If Bit1 Then out += 1
        If Bit2 Then out += 2
        If Bit3 Then out += 4
        If Bit4 Then out += 8
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function FromBinary(Binary As Byte(), ByteIndex As Long, BitIndex As Long) As Bit8
        Dim output As Bit8 = Nothing
        If BitIndex > 7 Then
            ByteIndex += Math.Floor(BitIndex \ 8)
            BitIndex = BitIndex Mod 8
        End If
        Dim byte1 As Bit8 = Binary(ByteIndex)
        Dim byte2 As Bit8 = 0
    ...
    End Sub
#Region "Operators"
    Public Shared Narrowing Operator CType(ByVal x As Byte) As Bit8
        Return New Bit8(x)
    End Operator
    Public Shared Widening Operator CType(ByVal x As Bit8) As Byte
        Return x.Value
    End Operator
    Public Shared Operator +(ByVal x As Bit8, ByVal y As Bit8) As Bit8
        Return x.Value + y.Value
    End Operator
    Public Shared Operator =(ByVal x As Bit8, ByVal y As Byte) As Boolean
        Return x.Value = y
    End Operator
    Public Shared Operator <>(ByVal x As Bit8, ByVal y As Byte) As Boolean
        Return x.Value <> y
...
Public Class BitOperations
    ' ''' <summary>
    ' ''' Depricated; use ShfitRightPMD
    ' ''' </summary>
    ' ''' <param name="Binary"></param>
    ' ''' <param name="startbit"></param>
    ' ''' <param name="bits"></param>
    ' ''' <returns></returns>
    ' ''' <remarks>Credit to Prof9 for writing, evandixon for adapting it to this program</remarks>
    'Public Shared Function DecodeBytes(ByVal Binary As Byte(), startbit As Integer, bits As Integer) As Byte()
    '    'Array.Reverse(Binary)
    '    'For x As Integer = 1 To startbit
    '    '    ShiftRight(Binary)
    '    'Next
    '    'Array.Reverse(Binary)
    '    'Return SubByteArr(Binary, 0, bits / 8)
    '    If startbit = 0 Then
    '        Return Binary
    '        Exit Function
    '    End If

    '    Dim src As New IO.MemoryStream(Binary)
    ...
    '        b >>= 1
    '        'Check to see if Bit1 of a is 1
    '        If (a And 1) = 1 Then
    '            'Set Bit8 of b to 1
    '            b = b Or &H80
    '        End If
    ...
    Public Shared Function ShiftRightPMD(ByVal bytes As Byte(), bits As Integer, StartIndex As Integer, ByteCount As Integer) As Byte()
        If bits = 0 Then
            Return SubByteArr(bytes, StartIndex, StartIndex + ByteCount - 1)
        Else
            Dim out As Byte() = SubByteArr(bytes, StartIndex, StartIndex + ByteCount - 1)
            For count As Integer = 0 To ByteCount - 1
