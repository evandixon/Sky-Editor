...
Namespace FileFormats
    Public Class BGP
        ...
        Private ReadOnly Property ImageData As Byte()
            Get
                Return GenericArrayOperations(Of Byte).CopyOfRange(RawData, &HC40, RawData.Length - 1)
            End Get
        End Property
        Private ReadOnly Property MapData As Byte()
            Get
                Return GenericArrayOperations(Of Byte).CopyOfRange(RawData, &H420, &HA1F + 1)
            End Get
        End Property
        Private ReadOnly Property PaletteData As Byte()
            Get
                Return GenericArrayOperations(Of Byte).CopyOfRange(RawData, &H20, &H420)
            End Get
        End Property
        ...
        End Function
        ''' <summary>
        ''' Saves the image to the temporary directory, using Filename as part of the new filename.
        ''' </summary>
        ...
        Public Async Function GetTempImageURI(Filename As String) As Task(Of String)
            ...
            End If
            Dim uri As String = String.Format(directory & "\TempBPG{0}{1}.png", Filename, Guid.NewGuid.ToString)
