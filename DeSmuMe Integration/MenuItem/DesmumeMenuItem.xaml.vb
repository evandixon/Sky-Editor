...
Public Class DesmumeMenuItem
    ...
    Function RunRom(Optional SuppressErrorMessage As Boolean = False) As Task
        ...
        '        Await DirectCast(m.Save, GenericNDSRom).RePack(romDirectory & "\current.nds")
        ...
        '                End If
        '                m.Save.Save(IO.Path.Combine(IO.Path.GetDirectoryName(Loader.GetBestVersionExecutableName), "Battery/" & IO.Path.GetFileNameWithoutExtension(x.RomName) & ".dsv"))
        '                Using dsv As New IO.FileStream(IO.Path.Combine(IO.Path.GetDirectoryName(Loader.GetBestVersionExecutableName), "Battery/" & IO.Path.GetFileNameWithoutExtension(x.RomName) & ".dsv"), IO.FileMode.Open, IO.FileAccess.ReadWrite, IO.FileShare.Read)
